# 最新框架已变更为 Ciyon
请移步到 [ciyon.ciy.cn](http://ciyon.ciy.cn/)  
Ciyon 是一款”无第三方依赖”的全栈框架。  
包括 Uniapp Vue3移动端（未引用任何第三方库）、PHP8无依赖后端、Golang零插件后端。  
已实现OpenAI接口、Web3D、区块链、代码生成、文档生成、完整后端、常见通用业务能力，支持2025年版个税代扣缴。  
包含AI Skills 和 MCP自动化。  
  
互助贡献，承诺完成一项任务，加入社群即可获取完整源码。  
...
项目探索期，可用PHP/Python编写代码。项目高并发期，部分并发高的函数用Golang部分改写或全部重写。
